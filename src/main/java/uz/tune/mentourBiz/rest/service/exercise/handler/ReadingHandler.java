...
public class ReadingHandler implements SectionHandler {
    ...
    public ResLessonSection getSectionProgress(Student student, Unit unit) {
        List<ExerciseTask> tasks = exerciseTaskRepository.findAllByUnit_UuidAndSectionTypeAndStatus(unit.getUuid(), LessonSectionType.READING, ExerciseTaskStatus.ACTIVE);
        ...
        if (totalPossible == 0) return new ResLessonSection(LessonSectionType.READING, "READING", 0, false);

        long totalEarned = answerRepo.sumTotalEarnedScoreByStudentAndUnitAndSectionType(
                student.getId(), unit.getUuid(), LessonSectionType.READING);

        int percent = (int) Math.round((double) totalEarned / totalPossible * 100);
        return new ResLessonSection(LessonSectionType.READING, "READING", Math.min(percent, 100), false);
    }
}
