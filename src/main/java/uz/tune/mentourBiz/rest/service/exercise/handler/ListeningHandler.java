...
public class ListeningHandler implements SectionHandler {
    ...
    public ResLessonSection getSectionProgress(Student student, Unit unit) {
        List<ExerciseTask> tasks = exerciseTaskRepository.findAllByUnit_UuidAndSectionTypeAndStatus(unit.getUuid(), LessonSectionType.LISTENING, ExerciseTaskStatus.ACTIVE);
        ...
        if (totalPossible == 0) return new ResLessonSection(LessonSectionType.LISTENING, "LISTENING", 0, false);

        long totalEarned = answerRepo.sumTotalEarnedScoreByStudentAndUnitAndSectionType(
                student.getId(), unit.getUuid(), LessonSectionType.LISTENING);

        int percent = (int) Math.round((double) totalEarned / totalPossible * 100);
        return new ResLessonSection(LessonSectionType.LISTENING, "LISTENING", Math.min(percent, 100), false);
    }
}