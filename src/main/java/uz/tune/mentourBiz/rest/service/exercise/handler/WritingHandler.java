...
public class WritingHandler implements SectionHandler {
    ...
    public ResLessonSection getSectionProgress(Student student, Unit unit) {
        List<ExerciseTask> tasks = exerciseTaskRepository.findAllByUnit_UuidAndSectionTypeAndStatus(unit.getUuid(), LessonSectionType.WRITING, ExerciseTaskStatus.ACTIVE);
        ...
        if (totalPossible == 0) return new ResLessonSection(LessonSectionType.WRITING, "WRITING", 0, false);

        Long totalEarned = answerRepo.sumTotalEarnedScoreByStudentAndUnitAndSectionType(
                student.getId(), unit.getUuid(), LessonSectionType.WRITING);

        int percent = (int) Math.round((double) (totalEarned != null ? totalEarned : 0) / totalPossible * 100);
        return new ResLessonSection(LessonSectionType.WRITING, "WRITING", Math.min(percent, 100), false);
    }
}